...
function fnConstEditCheck() {
	...
	if (!fnYMDCheck("正しい買取決済日付", form.purchaseDT)) {} // 後ほどチェックする
	if (!fnYMDCheck("正しい工期開始日付", form.workStartDT)) {} // 後ほどチェックする
	if (!fnYMDCheck("正しい工期終了日付", form.workEndDT)) {} // 後ほどチェックする
	if (!fnYMDCheck("正しい電気水道開栓日付", form.lineOpenDT)) {} // 後ほどチェックする
	if (!fnYMDCheck("正しい電気水道閉栓日付", form.lineCloseDT)) {} // 後ほどチェックする
	if (!fnYMDCheck("正しい電気水道開栓連絡日", form.lineOpenContactDT)) {} // 後ほどチェックする
	if (!fnYMDCheck("正しい電気水道閉栓連絡日", form.lineCloseContactDT)) {} // 後ほどチェックする
